...
//
//    var map = MultiKeyMaps.CreateMultiKeyDictionary<Employee<Integer>, Iterable<Employee<Integer>>, String>(subKeyComparer, keyComparer, strategy);
...
//  PopulateDictionary(dict as Map<Iterable<T>, V>, recordCount, keySize, subKeyConverter, valueConverter);
//  var keys = dict.Keys.Where((x, index) => index % 3 == 0).Take(searchCount).ToList();
//  (var partKeys, var positivePositions, var positions) = PopulatePartialKeys((dict as Map<Iterable<T>, V>).Keys, searchCount);
...
//  {
//      map.put(Stream.iterate(i, j -> j + 1).limit(keySize - i).collect(Collectors.mapping(subKeyConverter, Collectors.toList())),
...
//  List<Integer> mixedPositions = positivePositions.stream().map(x -> (x % 2 == 0) ? -1 : x).collect(Collectors.toList());
//
//  Collectors.
//  //Streams.stream(keys).filter(Predicates.)
//  
//  Streams.zip(Streams.stream(keys), , function)  .mapWithIndex(Streams.stream(keys), (key, index) -> (index % nthKey == 0) ? 
//  
//  
//  List<Iterable<T>> partKeys = StreamSupport.stream(keys.spliterator(), false).collect(Collectors.   filter(e((x, index) => index % nthKey == 0)
//      .Take(count)
...
////
////          Function<Integer, Employee<Integer>> subKeyConverter = x => new Employee<Integer>(String.Join("", Enumerable.Range(x, 10)), x);
